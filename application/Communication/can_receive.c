...
/* 是否收到过对方帧，避免上电初期误判为在线 */
static volatile uint8_t  board_comm_received = 0;
...
{
    if (board_comm_received == 0)
        return 0;   /* 还没收到过任何帧，判为离线 */
